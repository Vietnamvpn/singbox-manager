...
function show_config_menu() {
    ...
    case $cchoice in
        ...
        3)
            ...
            jq '(.outbounds[] | select(.tag == "direct")) += {"domain_strategy": "prefer_ipv6"}' "$CONFIG_FILE" > "${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
            echo -e "${GREEN}Đã bổ sung 'domain_strategy': 'prefer_ipv6' cho outbound direct.${NC}"
            check_and_restart
            read -p "Nhấn Enter để tiếp tục..." && show_config_menu
            ;;
        4)
